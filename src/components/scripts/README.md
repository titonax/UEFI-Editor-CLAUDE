...
| `setupDataFlags.ts` | Decodes the SetupData control flag byte shown as "Access Level" (bit 0 = shown, the AMIBCP Show/Hide switch; bit 3 = interactive; bits 5/6 tentative) and reports a clear bit 0 as a SetupData-level hide. Evidence in `docs/ami/setupdata-control-flags.md`. |
